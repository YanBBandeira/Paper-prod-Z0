c     TO COMPILE: gfortran -fdefault-real-8 -fdefault-double-8 -fno-align-commons vegas-sig-tot.f 
      program VEGAS_DY_CS
      implicit none
...
c     -----------------------------------------------------------------c
      ...
      if(iDoHist.eq.1.or.iDoHist.eq.0) then 
            aintegrand = (aintegral*units*ktp*ktm)
c            !! I'm not 100% sure that should be divided by 2*pi*pt
      endif
...
c     -----------------------------------------------------------------     
      if(y.gt.y_min.and.y.lt.y_max) then 
            ...
            sig_y(iy) = sig_y(iy) + aintegrand/dy!*wgt/dy
ctest            write(*,*) iy, ny
            endif
      endif
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
            ...
            sig_pt(ipt) = sig_pt(ipt) + aintegrand/dpt!*wgt/dpt
            endif
      endif
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
...
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
            ...
            sig_ypt2(ipt) = sig_ypt2(ipt) + aintegrand/(dpt*dy)!*wgt/dpt
                  endif
            endif
      endif
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
            ...
            sig_ypt3(ipt) = sig_ypt3(ipt) + aintegrand/(dpt*dy)!*wgt/dpt
                  endif
            endif
      endif
...
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
            ...
            sig_ypt4(ipt) = sig_ypt4(ipt) + aintegrand/(dpt*dy)!*wgt/dpt
                  endif
            endif
      endif
...
c     -----------------------------------------------------------------
      if(pt.gt.pt_min.and.pt.lt.pt_max) then 
            ...
            sig_ypt5(ipt) = sig_ypt5(ipt) + aintegrand/(dpt*dy)!*wgt/dpt
                  endif
            endif
      endif
...
c     -----------------------------------------------------------------
      if(Ml.gt.m_min.and.Ml.lt.m_max) then
            if(im.gt.0.and.im.le.nm) then
                  sig_m(im) = sig_m(im) + aintegrand/dm!*wgt/dm
            endif 
      endif
