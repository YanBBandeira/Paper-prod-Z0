...
c     ------------------------------------------------------------------
      ...
      iset = 400002 !KS-2013-non-linear 
      call TMDinit(iset)
      call TMDset(iset)
...
c     ==================================================================
      ...
      open(unit=11,file="DatFiles/ksnonlinear_grid.dat", 
...
C     Usar as flags -O3 -fopenmp -march=native -ffast-math -funroll-loops
      ...
      y(iy) = y_min + (iy-1)*dy
            ...
            do ipt = 1,nPoints
      pt(ipt) = 10.d0**(pt_min + (ipt-1)*dpt) - .5d0
...
cOPT      ugd = (pre_ugd*F_KS(x2,akt))/akt2
      ugd = F_KS(x2,akt)/akt2
