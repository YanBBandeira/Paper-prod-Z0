      ...
      DIMENSION AA(1), BB(1), WK(1100000)
      external TestIntegrand, DGAUSS
      !-----------------------------------------------------
      ! Preparação
      !-----------------------------------------------------
...
c     * RELERR,NFNEVL,IFFAIL)
      ...
      Result = DGAUSS(TestIntegrand,2.d0,4.5d0,1.d-5)

      dsigma = Gevtopb*Result
      write(*,*) ptZ, dsigma
      write(11,'(F10.4,2X,E12.5)') ptZ, dsigma
      end do
...
ctest      write(*,*)'Decay: ',Result,DecayWidth,Branch,InvariantMassDist
      ...
      ! x: M, y: y_Z
      !-----------------------------------------------------
      function TestIntegrand(yVar)
      use globals
      USE parameters
      implicit none
      INTEGER NN 
      DOUBLE PRECISION TestIntegrand, yVar, HadronicCrossSection,
...
c      HadronicCrossSection = DileptonDecay(MZ)
c     &      *FuncPartonLevelSigma(yZ, ptZ,MZ)
      HadronicCrossSection = FuncPartonLevelSigma(yZ, ptZ,MZ)
      
      TestIntegrand = HadronicCrossSection*2.0d0*pi*pt
