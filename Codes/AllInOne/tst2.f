      ...
      double precision Gevtopb, DileptonDecay
      double precision TestIntegrand,Result,dsigma
      DOUBLE PRECISION AA,BB,WK,EPS,RELERR,DGAUSS
      INTEGER          N,IWK,IMINPTS,IMAXPTS,IFFAIL,NFNEVL
      DIMENSION AA(2), BB(2), WK(1100000)
      external TestIntegrand, DGAUSS, DileptonDecay
      !-----------------------------------------------------
      ! Preparação
      !-----------------------------------------------------
      ...
      do ipt = 1, nPoints
      yZ= y_min + (ipt-1.d0)*dy
      y = yZ    
c      CALL DADMUL(TestIntegrand,NN,AA,BB,
c     * IMINPTS,IMAXPTS,EPS,WK,IWK,Result,
c     * RELERR,NFNEVL,IFFAIL)
     
      Result = DGAUSS(TestIntegrand,0.d0,1.0d0,1.d-5)

      dsigma = Gevtopb*Result
      write(*,*) yZ, dsigma
      write(11,'(F10.4,2X,E12.5)') yZ, dsigma
      end do
      
      end program z_y_distribution
     ...
     & (160.d0/3.d0)*(sin2**2.d0) - 40.d0*(sin2) + 21.d0)

      Branch = 3.33d0/100.d0
...
c      HadronicCrossSection = jac*DileptonDecay(MZZ)
c     &      *FuncPartonLevelSigma(yZ, ptZ,MZZ)
      HadronicCrossSection = FuncPartonLevelSigma(yZ, ptZ,MZ)     
      TestIntegrand=HadronicCrossSection*2.0d0*pi*ptz/((1.0d0-u)**2.d0)
ctest      write(*,*) HadronicCrossSection, ptz, yz
      return
      end
      ...
      VarJacobian = x1!(2.d0/rs)*DSQRT(M2 + pt2)*DCOSH(y)
      preIntegral = 1.d0!x1/(x1 + x2)
...
ctest      write(*,*) 'Pre-terms: ', preTerms, gfv2, gfa2, mf, M2, pt, z
      ...
      IMAXPTS = 50000   !Max number of points
      EPS = 1.d-4          !Numerical precision    
      IWK = 110000        !Work array dimension
...
c     ==================================================================
      ...
      tau2  = pt2 + z2*akt2 - 2.d0*z*pt*akt*Dcos(atheta) 
