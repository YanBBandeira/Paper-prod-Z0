...
def TestIntegrand(MZZ, yZ, ptZ):
    ...
    Result = DecayTerm * HadronicCS * (2.0 * MZZ) * np.pi *ptZ 
...
def dsigma_dydpt(pt,yVar):
    def integrand(Mz):
        return TestIntegrand(Mz,yVar,pt)

    result, error = quad(integrand,M_MIN, M_MAX, epsabs=1e-4,
        epsrel=1e-4 )
    return result
# --- 5. PROGRAMA PRINCIPAL (Substituindo z_pt_distribution) ---

def compute_dsigma_dptdy_for_bin(ymin, ymax, idx):
    ...
    # Escolha o y central do bin:
    y_center = 0.5*(ymin + ymax)

    print(f"\n--- Calculando d²σ/(dpT dy) no bin {ymin} < y < {ymax} ---")
    print(f"Usando y_central = {y_center}")
    ...
    for i in range(NPT):
        ...
        # d²σ/(dpT dy)
        val = dsigma_dydpt(pt, y_center)

        # converte para pb / GeV
        val_pb = val * GEV_TO_PB

        pt_vals.append(pt)
        sigma_vals.append(val_pb)

        print(f"pt={pt:.3f} GeV, d²σ/(dpT dy)={val_pb:.4e} pb/GeV")

    fname = f"dsig_dptdy_ybin_{idx}.dat"
    np.savetxt(fname, np.column_stack([pt_vals, sigma_vals]),
               fmt="%.6e  %.6e")
...
if __name__ == "__main__":
    ...
    for i, (ymin, ymax) in enumerate(y_bins, start=1):
        compute_dsigma_dptdy_for_bin(ymin, ymax, i)
...
for i, (ymin, ymax) in enumerate(y_bins, start=1):
    ...
    # Carrega o arquivo correspondente ao bin
    data = np.loadtxt(f"dsig_dptdy_ybin_{i}.dat")

    pt  = data[:,0]
    ds2 = data[:,1]   # d^2sigma/(dpT dy)

    label = rf"${ymin} < y < {ymax}$"

    plt.plot(pt, ds2, color=colors[i-1], linewidth=2, label=label)
...
plt.ylabel(r"$d^2\sigma/(dp_T\,dy)$ [pb/GeV]", fontsize=15)

plt.title(r"Distribuições de $p_T$ do $Z^0$ por intervalos de rapidez", fontsize=16)
